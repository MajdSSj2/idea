...
<x-modal name="{{$idea->exists ? 'edit-idea' : 'create-idea'}}" title="{{$idea->exists ? 'Edit Idea' : 'New idea'}}">
    <form
        x-data="{
                    status: @js($idea->exists ? $idea->status->value : 'pending'),
                    newLink: '',
        ...
        method="POST"
        action="{{$idea->exists ? route('idea.update') : route('idea.store')}}"
    ...
    @if($idea->image_path)
    <form method="POST" action="{{route('idea.image.destroyImage', $idea)}}" id="delete-image-form">
        @csrf
        @method('DELETE')
    </form>
    @endif
